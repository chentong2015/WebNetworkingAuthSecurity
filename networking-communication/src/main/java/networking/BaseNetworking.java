...
public class BaseNetworking {
    ...
    //    Physical Con: sent to the computer from other hosts on the internet will through that connection
    //    Port number : route the date to the application
    //    数据通过网络发送, 在接收时会通过port端口号来定位数据要传输的目的地APP, 端口号可能被占用
